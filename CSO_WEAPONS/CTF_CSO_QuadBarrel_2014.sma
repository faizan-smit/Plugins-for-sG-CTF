...
const PriceMoney = 9999;
const PriceAdrenaline = 70;
...
public plugin_init()
{
	...
	// CMD
	register_clcmd("say /quadbarrel", "Get_QuadBarrel")
}
...
public Get_QuadBarrel(id)
{
	...
	if(cs_get_user_money(id) >= PriceMoney && jctf_get_adrenaline(id) >= PriceAdrenaline) {
		// native or something for get item //
	...
	new newMoney = cs_get_user_money(id) - PriceMoney;
	cs_set_user_money(id, newMoney, 1);

	new newAdrenaline = jctf_get_adrenaline(id) - PriceAdrenaline;
	jctf_add_adrenaline(id, newAdrenaline);
	...
	message_end()
		...
		client_print(id, print_chat, "[CTF] You need 70 Vodka and $9999 to buy this weapon!")
	}
